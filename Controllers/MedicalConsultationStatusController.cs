...
public class MedicalConsultationStatusController : ControllerBase
{
    ...
    [HttpGet]
    public async Task<IActionResult> GetMedicalConsultationStatuses()
    {
        var query = new GetMedicalConsultationStatusesQuery();
        var medicalConsultationStatuses = await _mediator.Send(query);
        return Ok(medicalConsultationStatuses);
    }
